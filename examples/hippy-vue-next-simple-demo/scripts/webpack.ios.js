...
module.exports = {
  ...
  resolve: {
    ...
    alias: (() => {
      ...
      const hippyVueRuntimeCorePath = path.resolve(__dirname, '../../../packages/hippy-vue-next-simple/node_modules/@vue/runtime-core');
      ...
      // Remove the section if you don't use it
      const hippyVueNextPath = path.resolve(__dirname, '../../../packages/hippy-vue-next-simple/dist');
      if (fs.existsSync(path.resolve(hippyVueNextPath, 'index.js'))) {
        console.warn(`* Using the @hippy/vue-next-simple in ${hippyVueNextPath} as @hippy/vue-next-simple alias`);
        aliases['@hippy/vue-next-simple'] = hippyVueNextPath;
      } else {
        console.warn('* Using the @hippy/vue-next-simple defined in package.json');
      }
