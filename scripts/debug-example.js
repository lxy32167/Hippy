...
const cmdExample = 'please execute command like \'pnpm run debugexample hippy-react-demo dev\' or \'pnpm run debugexample hippy-react-demo debug\'';
...
if (!fs.existsSync(path.resolve(DEMO_PATH, 'node_modules'))) {
  console.error(`❌ ${example} dependencies have not been installed, please execute 'pnpm run init:example ${example}' first.`);
  process.exit(1);
  return;
}

console.log(`Start to start ${example} with cmd 'pnpm run hippy:${cmd}'.`);
